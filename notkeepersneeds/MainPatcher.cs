			...
			if (File.Exists(cfgPath)) {
				...
				foreach (string line in lines) {
					...
					if (pair.Length > 1) {
						...
						else if (key == "DullInventoryMusic") {
							if (pair[1] == "1" || pair[1].ToLower() == "true") {
								options_.DullInventoryMusic = true;
							}
						}
						else if (key == "HealthRegen") {
							if (pair[1] == "1" || pair[1].ToLower() == "true") {
								options_.HealthRegen = true;
							}
						}
						else if (key == "HealIfTired") {
							if (pair[1] == "1" || pair[1].ToLower() == "true") {
								options_.HealIfTired = true;
							}
						}
						else {
							float value = 0;
							if (float.TryParse(pair[1], out value)) {
								switch (key) {
									...
									case "HealthRegenPerSecond":
										if (value > 0) {
											options_.HealthRegenPerSecond = value;
										}
										break;
									case "SprintSpeed":
										if (value > 0) {
									...
									case "InflationAmount":
										options_.InflationAmount = value;
										break;
									case "OrbsMult":
										if (value < 0)
											value = 0;
